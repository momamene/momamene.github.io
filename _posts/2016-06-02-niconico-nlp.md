...
위에서 학습한 코멘트 카테고리 식별기는 출력층의 하나 전의 층을 제거하면 (카테고리 식별이 필요한)코멘트의 의미를 표현하는 특징(Feature)이 되는 것으로도 해석할 수 있습니다.  
...
이 구조는 간단한 어플리케이션으로 하는 것도 가능하고 [Jubatus](http://jubat.us/ja/)라는 온리인 분류기를 사용하면 코멘트의 특징량을 ID를 붙여 던져져 LSH나 minhash와 같은 근사 최근 주변 탐색으로 격납된 코멘트 안 특징량이 가까운 코멘트를 습득할 수 있습니다. [Jubarecommender](http://jubat.us/ja/api_recommender.html)를 사용했습니다.

![](https://qiita-image-store.s3.amazonaws.com/0/8954/e22291c5-8ba8-af91-62e4-8f4848794ad4.png)

이번에는 사전에 일괄로 니코니코 데이터세트의 코멘트 데이터의 특징량을 Jubatus에 저장하겠습니다. 이번에는 하진 않지만 이 작업으로 Jubatus도 온라인으로 후보를 추가할 수 있고 Chainer도 온라인으로 추가로 학습할 수 있기 때문에 실시간 학습이라는 면에서 재미있을지도 모릅니다. (학습률 조정을 어떻게 할지에 대한 과제는 있을 것 같습니다만)

### 코멘트 제안 예시

위의 하얀 직접 입력한 폼이 인력 코멘트, 아래의 「근방 투고」가 유사한 코멘트 입니다.
...
「お疲つかれ様さま」과 「乙おつ」가 동일한 특징을 가진 것을 알 수 있습니다.
...
이 국소성으로 부터 코멘트의 의미를 학습하는 것을 생각할 수 있습니다. 「의미적 거리가 그대로 특징량 사이의 거리가 되는 특징량」을 출력하는 네트워크를 목표로 하고 있습니다.  
...
loss function은 chainer v1.5부터 추가된 [contrasive loss](http://chainer.readthedocs.org/en/stable/reference/functions.html?highlight=contrastive#chainer.functions.contrastive)가 사용할 수 있을 것 같은데 이것을 적용할 수 있을 것 같습니다.(이 Function을 사용하지 않아도 Variable로 공통의 처리는 사용할 수 있지만)  
이 방법으로 실험 결과가 나오면 다시 어딘가에서 기사를 쓰려고 합니다.

## [덤] contrative loss와 상대적 관계의 학습에 대하여(일반적인 이야기)

contrastive loss는 y_n이 1(동일한 클래스)이라면 벡터강의 거리가 loss가 되며 0(다른 클래스)라면 지정한 margin보다 가까웠던 만큼의 거리가 loss(margin 이상 떨어진다면 loss가 0)입니다. 즉, 「동일한 클래스라면 가능한 만큼 뭉치고 다른 클래스라면 margin 이상 떨어진다 」처럼 최적화 됩니다.
...
일반적으로 N클래스 분류의 문제는 출력층을 N차원으로 하여 이것에 [softmax cross entropy](http://chainer.readthedocs.org/en/stable/reference/functions.html#chainer.functions.softmax_cross_entropy)를 적용하여 입력에 대하여 「어느 클래스에 가장 가까운가」를 예측하는 방식으로 학습합니다. 이것은 최초에 클래스의 갯수를 알고 있음을 전제하는 모델입니다.  
...
단지 [4096차원을 t-SNE로 2차원으로 떨어트려 가시화하는 것](http://cs.stanford.edu/people/karpathy/cnnembed/)을 봐도 꽤 깨끗하게 클래스가 떨어져있고 저차원에도 많은 차원의 특징을 표현하는 것이 충분히 가능하다고 느낍니다.  
...
이 방법을 사용하는 경우 단순히 랜덤하게 짝을 모으면 짝의 군의 의 다른 클래스가 되어 버리기 때문에 여기서는 각각 균형있게 학습 데이터의 짝을 만들 필요가 있습니다.
...
드완고(niconico를 서비스하고 있는 회사)에서는 "크리스"라는 이름의 수십대의 [Titan X](http://www.nvidia.co.jp/object/geforce-gtx-titan-x-jp.html)4장 꽂은 노드들이 있습니다.(안의 2대는 1TB의 메모리)  
드완고에서는 이 크리스의 환경을 코멘트 해석 뿐만아니라 일러스트 해석 등에 사용하고 있습니다.  
